...
class ProcessadorNotaFiscal(object):
        ...
        if nfe.infNFe.transp.transporta.CNPJ.valor or nfe.infNFe.transp.transporta.CPF.valor:
            ...
            if nfe.infNFe.transp.transporta.CNPJ.valor:
                transportadoras = [
                    ...
                    if t.cpf_cnpj_apenas_digitos == nfe.infNFe.transp.transporta.CNPJ.valor
                ]
            elif nfe.infNFe.transp.transporta.CPF.valor:
                transportadoras = [
                    t
                    for t in Transportadora.objects.filter(tipo_pessoa="PF")
                    if t.cpf_cnpj_apenas_digitos == nfe.infNFe.transp.transporta.CPF.valor
                ]
    ...
    def verificar_configuracao(self):
        ...
        try:
            ...
            return self.salvar_mensagem(
                message="Erro ao tentar ler o certificado, verifique se sua senha está correta.", erro=True
            )
    ...
    def validar_nota(self, nota_obj):
        ...
        if ErrosValidacaoNotaFiscal.objects.filter(nfe=nota_obj).filter(tipo="0").count():
            ...
            return self.salvar_mensagem(
                message='Erros de validação encontrados. Verifique a tab "Validação" para mais detalhes.', erro=True
            )
        else:
        ...
        if not nota_obj.estado:
            ...
            try:
                ...
                # HTTP 403 - Acesso proibido
                    ...
                    e.tipo = "0"
                    e.descricao = (
                        "Erro ao enviar nota, verifique a versão do seu aplicativo e a validade do seu certificado."
                    )
                    e.save()
                    self.salvar_mensagem(message=e.descricao, erro=True)
    ...
    def consultar_cadastro(self, empresa, salvar_arquivos):
        ...
        if not empresa.uf_padrao:
            ...
            try:
                ...
                return self.salvar_mensagem(message="Erro de autenticação: {}".format(e), erro=True)

    def inutilizar_notas(
        self, empresa, ambiente, modelo, serie, numero_inicial, numero_final, justificativa, salvar_arquivos
    ):
        self.nova_nfe = nf_e()
    ...
    def efetuar_manifesto(
        self, chave, cnpj, ambiente, tipo_manifesto, justificativa, ambiente_nacional, salvar_arquivos
    ):
        self.nova_nfe = nf_e()
