...
from djangosige.apps.cadastro.models import COD_UF
...
from djangosige.apps.cadastro.models import PessoaFisica
from djangosige.apps.cadastro.models import PessoaJuridica
from djangosige.apps.cadastro.models import Produto
from djangosige.apps.cadastro.models import Telefone
from djangosige.apps.cadastro.models import Transportadora
from djangosige.apps.cadastro.models import Unidade
...
from djangosige.apps.fiscal.models import ICMS
from djangosige.apps.fiscal.models import ICMSSN
from djangosige.apps.fiscal.models import IPI
from djangosige.apps.fiscal.models import PIS
from djangosige.apps.fiscal.models import AutXML
from djangosige.apps.fiscal.models import ConfiguracaoNotaFiscal
from djangosige.apps.fiscal.models import ErrosValidacaoNotaFiscal
from djangosige.apps.fiscal.models import GrupoFiscal
from djangosige.apps.fiscal.models import ICMSUFDest
...
from djangosige.apps.fiscal.models import RespostaSefazNotaFiscal
...
class ProcessadorNotaFiscal:
    ...
    def verificar_configuracao(self):
        ...
        if not self.conf_nfe.arquivo_certificado_a1:
            return self.salvar_mensagem(message="Certificado A1 não encontrado.", erro=True)
        ...
        try:
            ...
            return self.salvar_mensagem(message="Erro ao tentar ler o certificado, verifique se sua senha está correta.", erro=True)
    ...
    def verificar_configuracao_nfe(self, nota_obj):
        ...
        try:
            ...
            e = ErrosValidacaoNotaFiscal(nfe=nota_obj)
            e.tipo = "0"
            e.descricao = "Erro ao tentar ler o certificado, verifique se sua senha está correta."
            e.save()
            return self.salvar_mensagem(message=e.descricao, erro=True)
            ...
            if processos["lote"].resposta.status in ("200", 200):
                ...
                if processos["lote"].resposta.cStat.valor in ("104", 104):
                    if len(processos["notas"]):
                        ...
                        # Denegada
                        elif proc.protNFe.infProt.cStat.valor in ("110", "301", "302", "303"):
        ...
        try:
            ...
            if processo.resposta.status in ("200", 200):
                if len(processo.resposta.retEvento):
                    ...
                    # vinculado a NF-e
                    if ret.infEvento.cStat.valor == "101" or ret.infEvento.cStat.valor == "135":
    ...
    def gerar_danfe(self, nota_obj):
        ...
        danfe = self.nova_nfe.gerar_danfe(
            proc_nfe, nome_sistema="PySIGNFe", leiaute_logo_vertical=self.conf_nfe.leiaute_logo_vertical, versao=nota_obj.versao, logo=logo_path)
    ...
    def consultar_cadastro(self, empresa, salvar_arquivos):
        ...
        if not empresa.uf_padrao:
            return self.salvar_mensagem(message="Estado do emitente não foi preenchido.", erro=True)
        if not empresa.cpf_cnpj_apenas_digitos:
            return self.salvar_mensagem(message="CNPJ do emitente não foi preenchido.", erro=True)
        try:
            processo = self.nova_nfe.consultar_cadastro(cert=self.info_certificado["cert"], key=self.info_certificado["key"], cpf_cnpj=empresa.cpf_cnpj_apenas_digitos, versao="3.10",
                                                        ambiente=2, estado=empresa.uf_padrao, contingencia=False, salvar_arquivos=salvar_arquivos, caminho=MEDIA_ROOT)

            self.processo = processo

            if processo.resposta.status in ("200", 200):
                if processo.resposta.infCons.cStat.valor and processo.resposta.infCons.cStat.valor < "200":
            ...
            return self.salvar_mensagem(message=f"Erro de autenticação: {e}", erro=True)
    ...
    def inutilizar_notas(self, empresa, ambiente, modelo, serie, numero_inicial, numero_final, justificativa, salvar_arquivos):
        ...
        processo = self.nova_nfe.inutilizar_faixa_numeracao(cnpj=empresa.cpf_cnpj_apenas_digitos, serie=serie, numero_inicial=numero_inicial, numero_final=numero_final, justificativa=justificativa,
                                                            cert=self.info_certificado["cert"], key=self.info_certificado["key"], versao="3.10", ambiente=int(ambiente), estado=empresa.uf_padrao, nfce=nfce, contingencia=False, caminho=MEDIA_ROOT)

        self.processo = processo

        if processo.resposta.status in ("200", 200):
            if processo.resposta.infInut.cStat.valor and processo.resposta.infInut.cStat.valor < "200":
    ...
    def consultar_nota(self, chave, ambiente, salvar_arquivos):
        ...
        try:
            ...
            return self.salvar_mensagem(message="Chave com código da UF incorreto.", erro=True)

        processo = self.nova_nfe.consultar_nfe(chave=chave, cert=self.info_certificado["cert"], key=self.info_certificado["key"], versao="3.10", ambiente=int(
    ...
    def baixar_nota(self, chave, ambiente, ambiente_nacional, salvar_arquivos):
        ...
        processo = self.nova_nfe.download_notas(cnpj=cnpj, lista_chaves=[chave ], ambiente_nacional=ambiente_nacional, cert=self.info_certificado["cert"], key=self.info_certificado["key"],
                                                versao="3.10", ambiente=int(ambiente), estado=uf, contingencia=False, caminho=MEDIA_ROOT)
    ...
    def efetuar_manifesto(self, chave, cnpj, ambiente, tipo_manifesto, justificativa, ambiente_nacional, salvar_arquivos):
        ...
        processo = self.nova_nfe.efetuar_manifesto(cnpj=cnpj, tipo_manifesto=tipo_manifesto, chave=chave, ambiente_nacional=ambiente_nacional, cert=self.info_certificado["cert"], key=self.info_certificado["key"], versao="3.10",
