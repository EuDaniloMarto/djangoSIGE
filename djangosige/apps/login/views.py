    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if associated_user:
                try:
                    if associated_user.email:
                        ...
                        if sended == 1:
                            ...
                            form.add_error(field=None, error="Erro ao enviar email de verificação.")
                            return self.form_invalid(form)
                    else:
                        form.add_error(field=None, error="Este usuário não cadastrou um email.")
                        return self.form_invalid(form)
                ...
                form.add_error(field=None, error="Usuário/Email: {} não foi encontrado na database.".format(data))
                return self.form_invalid(form)
    ...
    def post(self, request, uidb64=None, token=None, *args, **kwargs):
        ...
        if user is not None and default_token_generator.check_token(user, token):
            if form.is_valid():
                ...
                form.add_error(field=None, error="Não foi possivel trocar a senha. Formulário inválido.")
                return self.form_invalid(form)
        else:
            form.add_error(
                field=None,
...
class EditarPermissoesUsuarioView(SuperUserRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["user"] = user
        condition = reduce(operator.or_, [Q(codename__icontains=s) for s in ["add_", "change_", "view_", "delete_"]])
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not user.is_superuser:
            ...
            for nova_permissao_codename in request.POST.getlist("select_permissoes"):
                nova_permissao = Permission.objects.get(codename=nova_permissao_codename)
