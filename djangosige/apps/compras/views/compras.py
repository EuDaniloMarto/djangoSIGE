...
from djangosige.apps.compras.forms import ItensCompraFormSet, OrcamentoCompraForm, PagamentoFormSet, PedidoCompraForm
...
class AdicionarCompraView(CustomCreateView):
    ...
    def get(self, request, form_class, *args, **kwargs):
        ...
        return self.render_to_response(
            self.get_context_data(form=form, produtos_form=produtos_form, pagamento_form=pagamento_form)
        )
    ...
    def post(self, request, form_class, *args, **kwargs):
        ...
        return self.form_invalid(form=form, produtos_form=produtos_form, pagamento_form=pagamento_form)
...
class OrcamentoCompraVencimentoHojeListView(OrcamentoCompraListView):
    ...
    def view_context(self, context):
        context["title_complete"] = "ORÇAMENTOS DE COMPRA COM VENCIMENTO DIA " + datetime.now().date().strftime(
            "%d/%m/%Y"
        )
        context["add_url"] = reverse_lazy("compras:addorcamentocompraview")
        return context
