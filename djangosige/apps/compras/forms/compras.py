...
from djangosige.apps.compras.models import Compra, ItensCompra, OrcamentoCompra, PedidoCompra
...
class ItensCompraForm(forms.ModelForm):
    ...
    calculo_imposto = forms.CharField(
        widget=forms.TextInput(attrs={"class": "hidden", "disabled": True}), label="Cálc. Impostos", required=False
    )

    # IMPOSTO
    p_red_bc = forms.DecimalField(
        widget=forms.TextInput(attrs={"class": "modal-field", "disabled": True}), required=False
    )
