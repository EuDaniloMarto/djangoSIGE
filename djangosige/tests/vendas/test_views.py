...
class VendasAdicionarViewsTestCase(BaseTestCase):
    ...
    def test_add_orcamento_venda_view_post_request(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "vendas/orcamento_venda/orcamento_venda_list.html")
...
class VendasListarViewsTestCase(BaseTestCase):
    def test_list_orcamento_venda_view_deletar_objeto(self):
        ...
        self.check_list_view_delete(url=reverse("vendas:listaorcamentovendaview"), deleted_object=obj)

    def test_list_orcamento_venda_vencido_view_deletar_objeto(self):
        cli = Cliente.objects.order_by("id").last()
        obj = OrcamentoVenda.objects.create(cliente=cli, data_vencimento=datetime.now().date() - timedelta(days=1))
        self.check_list_view_delete(url=reverse("vendas:listaorcamentovendavencidoview"), deleted_object=obj)

    def test_list_orcamento_venda_vence_hoje_view_deletar_objeto(self):
        cli = Cliente.objects.order_by("id").last()
        obj = OrcamentoVenda.objects.create(cliente=cli, data_vencimento=datetime.now().date())
        self.check_list_view_delete(url=reverse("vendas:listaorcamentovendahojeview"), deleted_object=obj)

    def test_list_pedido_venda_view_deletar_objeto(self):
        cli = Cliente.objects.order_by("id").last()
        obj = PedidoVenda.objects.create(cliente=cli)
        self.check_list_view_delete(url=reverse("vendas:listapedidovendaview"), deleted_object=obj)

    def test_list_pedido_venda_atrasado_view_deletar_objeto(self):
        cli = Cliente.objects.order_by("id").last()
        obj = PedidoVenda.objects.create(cliente=cli, data_entrega=datetime.now().date() - timedelta(days=1))
        self.check_list_view_delete(url=reverse("vendas:listapedidovendaatrasadosview"), deleted_object=obj)

    def test_list_pedido_venda_entrega_hoje_view_deletar_objeto(self):
        cli = Cliente.objects.order_by("id").last()
        obj = PedidoVenda.objects.create(cliente=cli, data_entrega=datetime.now().date())
        self.check_list_view_delete(url=reverse("vendas:listapedidovendahojeview"), deleted_object=obj)

    def test_list_condicao_pagamento_view_deletar_objeto(self):
        obj = CondicaoPagamento.objects.create(n_parcelas=6)
        self.check_list_view_delete(url=reverse("vendas:listacondicaopagamentoview"), deleted_object=obj)
...
class VendasAcoesUsuarioViewsTestCase(BaseTestCase):
    ...
    def test_copiar_orcamento_venda(self):
        ...
        response = self.client.get(url, follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "vendas/orcamento_venda/orcamento_venda_edit.html")
