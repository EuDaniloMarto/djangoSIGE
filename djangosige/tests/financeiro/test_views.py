...
from djangosige.apps.financeiro.models import Entrada, MovimentoCaixa, PlanoContasGrupo, PlanoContasSubgrupo, Saida
...
class FinanceiroFluxoCaixaViewTestCase(BaseTestCase):
    ...
    def test_fluxo_caixa_get_request(self):
        ...
        self.check_user_get_permission(self.url, permission_codename=permission_codename)
    ...
    def test_fluxo_caixa_get_request_data_inicial(self):
        ...
        self.assertEqual(
            ...
            list(MovimentoCaixa.objects.filter(data_movimento__range=(data_inicial, data_inicial))),
        )
    ...
    def test_fluxo_caixa_get_request_data_final(self):
        ...
        self.assertEqual(
            list(response.context["movimentos"]),
            list(MovimentoCaixa.objects.filter(data_movimento__range=(data_final, data_final))),
        )
    ...
    def test_fluxo_caixa_get_request_datas_inicial_final(self):
        ...
        self.assertEqual(
            list(response.context["movimentos"]),
            list(MovimentoCaixa.objects.filter(data_movimento__range=(data_inicial, data_final))),
        )
    ...
    def test_fluxo_caixa_get_request_data_formato_errado(self):
        ...
        self.assertEqual(str(msgs[0]), "Formato de data incorreto, deve ser no formato DD/MM/AAAA")
...
class FinanceiroPlanoContasViewsTestCase(BaseTestCase):
    ...
    def test_deletar_grupo_plano_contas(self):
        obj = PlanoContasGrupo.objects.create(codigo="000001", tipo_grupo="0", descricao="Grupo Entrada Teste")
        self.check_list_view_delete(url=self.url, deleted_object=obj, context_object_key="all_grupos_entrada")
    ...
    def test_deletar_subgrupo_plano_contas(self):
        ...
        obj = PlanoContasGrupo.objects.get(codigo="000010", tipo_grupo="0", descricao="Subgrupo Entrada Teste")
        self.check_list_view_delete(url=self.url, deleted_object=obj, context_object_key="all_grupos_entrada")
...
class FinanceiroListarViewsTestCase(BaseTestCase):
    ...
    def test_list_conta_pagar_view_deletar_objeto(self):
        ...
        self.check_list_view_delete(url=reverse("financeiro:listacontapagarview"), deleted_object=obj)

    def test_list_conta_pagar_atrasada_view_deletar_objeto(self):
        obj = Saida.objects.create(status="1", data_vencimento=datetime.today() - timedelta(days=1))
        self.check_list_view_delete(url=reverse("financeiro:listacontapagaratrasadasview"), deleted_object=obj)

    def test_list_conta_pagar_hoje_view_deletar_objeto(self):
        obj = Saida.objects.create(status="1", data_vencimento=datetime.today())
        self.check_list_view_delete(url=reverse("financeiro:listacontapagarhojeview"), deleted_object=obj)

    def test_list_conta_receber_view_deletar_objeto(self):
        obj = Entrada.objects.create(status="1")
        self.check_list_view_delete(url=reverse("financeiro:listacontareceberview"), deleted_object=obj)

    def test_list_conta_receber_atrasada_view_deletar_objeto(self):
        obj = Entrada.objects.create(status="1", data_vencimento=datetime.today() - timedelta(days=1))
        self.check_list_view_delete(url=reverse("financeiro:listacontareceberatrasadasview"), deleted_object=obj)
    ...
    def test_list_conta_receber_hoje_view_deletar_objeto(self):
        ...
        self.check_list_view_delete(url=reverse("financeiro:listacontareceberhojeview"), deleted_object=obj)

    def test_list_recebimento_view_deletar_objeto(self):
        novo_movimento = MovimentoCaixa(data_movimento=datetime.today(), entradas=Decimal("120.00"))
        try:
            ultimo_mvmt = MovimentoCaixa.objects.filter(data_movimento__lt=novo_movimento.data_movimento).latest(
                "data_movimento"
            )
            novo_movimento.saldo_inicial = ultimo_mvmt.saldo_final
            novo_movimento.saldo_final = novo_movimento.saldo_inicial + Decimal("120.00")
        ...
        self.check_list_view_delete(url=reverse("financeiro:listarecebimentosview"), deleted_object=obj)
    ...
    def test_list_pagamento_view_deletar_objeto(self):
        novo_movimento = MovimentoCaixa(data_movimento=datetime.today(), saidas=Decimal("120.00"))
        try:
            ultimo_mvmt = MovimentoCaixa.objects.filter(data_movimento__lt=novo_movimento.data_movimento).latest(
                "data_movimento"
            )
        ...
        self.check_list_view_delete(url=reverse("financeiro:listapagamentosview"), deleted_object=obj)
...
class FinanceiroEditarViewsTestCase(BaseTestCase):
    ...
    def test_edit_recebimento_com_data_pagamento_igual_get_post_request(self):
        ...
        data["valor_total"] = locale.format("%.2f", Decimal(data["valor_total"]) + Decimal("20.00"), 1)
        data["valor_liquido"] = locale.format("%.2f", Decimal(data["valor_liquido"]) + Decimal("20.00"), 1)
        response = self.client.post(url, data, follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "financeiro/lancamento/lancamento_list.html")
        ...
        self.assertEqual(valor_saldo_final_antigo + Decimal("20.00"), movimento_editado.saldo_final)
    ...
    def test_edit_recebimento_com_data_pagamento_diferente_e_movimento_criado_get_post_request(self):
        ...
        movimento_editado = MovimentoCaixa.objects.get(data_movimento=nova_data_pagamento)
    ...
    def test_edit_recebimento_movimentar_caixa_true_get_post_request(self):
        ...
        # Verificar se movimento foi criado corretamente
        movimento_editado = MovimentoCaixa.objects.get(data_movimento=data_pagamento_futura)
...
class FinanceiroGerarLancamentoViewTestCase(BaseTestCase):
    ...
    def test_gerar_recebimento_a_partir_de_conta_receber_mesma_data_pagamento(self):
        ...
        obj = (
            ...
            .exclude(
                Q(movimento_caixa__isnull=True) | (Q(data_pagamento__isnull=True) & Q(data_vencimento__isnull=True))
            )
            .order_by("pk")
            .last()
        )
        ...
        data = {"contaId": obj.pk, "tipoConta": "0", "dataPagamento": data_pagamento.strftime("%d/%m/%Y")}
        response = self.client.post(self.url, data, follow=True)
        response_content = json.loads(response.content.decode("utf-8"))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response_content["url"], reverse("financeiro:editarrecebimentoview", kwargs={"pk": obj.id}))
    ...
    def test_gerar_pagamento_a_partir_de_conta_pagar_mesma_data_pagamento(self):
        ...
        if obj.data_pagamento:
            ...
            data_pagamento = obj.data_vencimento
        data = {"contaId": obj.pk, "tipoConta": "1", "dataPagamento": data_pagamento.strftime("%d/%m/%Y")}
        response = self.client.post(self.url, data, follow=True)
        response_content = json.loads(response.content.decode("utf-8"))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response_content["url"], reverse("financeiro:editarpagamentoview", kwargs={"pk": obj.id}))
...
class FinanceiroFaturarPedidoViewsTestCase(BaseTestCase):
    def test_faturar_pedido_venda(self):
        ...
        response = self.client.get(url, follow=True, HTTP_REFERER=reverse("financeiro:listalancamentoview"))
        self.assertEqual(response.status_code, 200)

        # Verificar se estoque dos produtos foi movimentado
        self.assertTrue(SaidaEstoque.objects.filter(observacoes="Saída de estoque pelo pedido de venda nº4").exists())
