...
from djangosige.cadastro.models import Categoria, Cliente, Empresa, Fornecedor, Marca, Produto, Transportadora, Unidade
...
class CadastroAdicionarViewsTestCase(BaseTestCase):
    ...
    def test_add_pessoa_post_request(self):
        for model in PESSOA_MODELS:
            ...
            response = self.client.post(url, pessoa_data, follow=True)
            self.assertFormError(response, "form", "nome_razao_social", "Este campo é obrigatório.")
    ...
    def test_add_categoria_post_request(self):
        ...
        data["categoria_desc"] = ""
        response = self.client.post(url, data, follow=True)
        self.assertFormError(response, "form", "categoria_desc", "Este campo é obrigatório.")
    ...
    def test_add_marca_post_request(self):
        ...
        data["marca_desc"] = ""
        response = self.client.post(url, data, follow=True)
        self.assertFormError(response, "form", "marca_desc", "Este campo é obrigatório.")
    ...
    def test_add_unidade_post_request(self):
        ...
        data["sigla_unidade"] = ""
        response = self.client.post(url, data, follow=True)
        self.assertFormError(response, "form", "sigla_unidade", "Este campo é obrigatório.")
...
class CadastroEditarViewsTestCase(BaseTestCase):
    def test_edit_pessoa_get_post_request(self):
        for model in PESSOA_MODELS:
            ...
            url = reverse("cadastro:editar{}view".format(model_name), kwargs={"pk": obj.pk})
            response = self.client.get(url)
            self.assertEqual(response.status_code, 200)
            data = response.context["form"].initial
            if model_name == "cliente":
                data["{}-limite_de_credito".format(response.context["form"].prefix)] = data["limite_de_credito"]
            ...
            # Assert form invalido
            data["{}_form-nome_razao_social".format(response.context["form"].prefix)] = ""
            response = self.client.post(url, data, follow=True)
            self.assertFormError(response, "form", "nome_razao_social", "Este campo é obrigatório.")
