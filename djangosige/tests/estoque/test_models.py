...
from djangosige.apps.estoque.models import EntradaEstoque, ItensMovimento, LocalEstoque, SaidaEstoque
...
class MovimentoEstoqueModelTestCase(TestCase):
    ...
    def test_saida_estoque_delete(self):
        ...
        produto = Produto.objects.create(codigo="123", descricao="produto", estoque_atual=100)
        local = LocalEstoque.objects.create(descricao="localTeste")
        # Adiciona produtos estocados
        produto_estocado = local.local_produto_estocado.create(produto=produto, quantidade=100)
