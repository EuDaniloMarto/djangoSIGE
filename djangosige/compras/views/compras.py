...
from djangosige.base.custom_views import (CustomCreateView, CustomListView,
                                          CustomUpdateView, CustomView)
from djangosige.cadastro.models import MinhaEmpresa
from djangosige.compras.forms import (ItensCompraFormSet, OrcamentoCompraForm,
                                      PagamentoFormSet, PedidoCompraForm)
from djangosige.compras.models import (ItensCompra, OrcamentoCompra, Pagamento,
                                       PedidoCompra)
from djangosige.estoque.models import (EntradaEstoque, ItensMovimento,
                                       ProdutoEstocado)
...
class OrcamentoCompraVencimentoHojeListView(OrcamentoCompraListView):
    ...
    def view_context(self, context):
        context["title_complete"] = (
            "ORÇAMENTOS DE COMPRA COM VENCIMENTO DIA "
...
class PedidoCompraEntregaHojeListView(PedidoCompraListView):
    ...
    def view_context(self, context):
        context["title_complete"] = (
            "PEDIDOS DE COMPRA COM ENTREGA DIA "
            + datetime.now().date().strftime("%d/%m/%Y")
        )
        context["add_url"] = reverse_lazy("compras:addpedidocompraview")
        return context
