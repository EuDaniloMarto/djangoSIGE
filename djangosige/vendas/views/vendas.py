...
from djangosige.vendas.forms import ItensVendaFormSet, OrcamentoVendaForm, PagamentoFormSet, PedidoVendaForm
...
class OrcamentoVendaVencimentoHojeListView(OrcamentoVendaListView):
    ...
    def view_context(self, context):
        context["title_complete"] = "ORÇAMENTOS DE VENDA COM VENCIMENTO DIA " + datetime.now().date().strftime(
            "%d/%m/%Y"
        )
        context["add_url"] = reverse_lazy("vendas:addorcamentovendaview")
        return context
